...
- Users cannot access files outside of the electron installer path during startup
- Certain web preferences are now not overriden by the ElectronHostWindowOptions
...
- pass "this" argument correctly when calling electron methods from frontend
...
- fix frontendURL option
- refactor into ElectronHost and ElectronApp
- Support Node 14
...
- Electron should registerFIleProtocol after app is ready
...
- run with nodeIntegration=false
...
- Removing query string from path (query strings are not supported in file protocol)
